...
public class JWTAuthorRsaJoseJImpl implements JWTAuthor {
    ...
    public String createJWT(String subject, Map<String, Object> claims) {
        ...
        claimsObj.setIssuer(env.getProperty("jwtIssuer")); // who creates the
                                                           // token and signs it
        claimsObj.setAudience(env.getProperty("jwtAudience")); // to whom the
                                                               // token is
                                                               // intended to be
                                                               // sent
        claimsObj.setExpirationTimeMinutesInTheFuture(
                Integer.parseInt(env.getProperty("jwtExpirationTimeMinutesInTheFuture"))); // time
                                                                                           // when
                                                                                           // the
                                                                                           // token
                                                                                           // will
        ...
        claimsObj.setNotBeforeMinutesInThePast(Integer.parseInt(env.getProperty("jwtNotBeforeMinutesInThePast"))); // time
                                                                                                                   // before
                                                                                                                   // which
                                                                                                                   // the
                                                                                                                   // token
                                                                                                                   ...
                                                                                                                   // (minutes
                                                                                                                   // ago)
        claimsObj.setSubject(subject); // the subject/principal is whom the
                                       // token is about

        for (Map.Entry<String, Object> claim : claims.entrySet()) {
            if (claim.getValue() instanceof List<?>) {
                claimsObj.setStringListClaim(claim.getKey(), (List<String>) claim.getValue());
            } else {
                claimsObj.setClaim(claim.getKey(), claim.getValue());
            }

            LOG.info("JWT with subject [" + subject + "] has claim [" + claim.getKey() + "] with value ["
                    + claim.getValue() + "]");
        }

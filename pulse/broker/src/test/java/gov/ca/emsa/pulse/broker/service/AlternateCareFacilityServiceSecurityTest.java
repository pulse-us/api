...
public class AlternateCareFacilityServiceSecurityTest extends BaseSecurityTest {
	...
	public void testGetAcfs() throws Exception {
		 ...
		 * 
		 * scenario 2a: user=ROLE_ORG_ADMIN, state=9009 (arbitrary), gets none. scenario
		 * 2b: user=ROLE_ORG_ADMIN, state=10, gets only 1 acf
		 * 
		...
		// scenario 1: user=ROLE_ADMIN, gets all acfs.
		TestUsers.setCurrentUser("ROLE_ADMIN", 999999L, 99999L); // the info doesn't matter for role_admin
		...
		// System.out.println("Matches = " + StringUtils.countMatches(out, "<item>"));
		assertTrue(StringUtils.countMatches(out, "<item>") == totalAcfs);

		// scenario 2a: user=ROLE_ORG_ADMIN, state=9009 (arbitrary), gets none.
		TestUsers.setCurrentUser("ROLE_ORG_ADMIN", 9009L, 100L); // the info doesn't matter for role_admin
		...
		TestUsers.setCurrentUser("ROLE_ORG_ADMIN", liferayStateIdUsedForTest, 10000L /* ignored */);
		result = mockMvc.perform(get("/acfs")).andExpect(status().isOk()).andReturn();
		...
		TestUsers.setCurrentUser("ROLE_ORG_ADMIN", 9009L, liferayAcfIdUsedForTest);
		result = mockMvc.perform(get("/acfs")).andExpect(status().isOk()).andReturn();
		...
		TestUsers.setCurrentUser("ROLE_ORG_ADMIN", liferayStateIdUsedForTest, liferayAcfIdUsedForTest);
		result = mockMvc.perform(get("/acfs")).andExpect(status().isOk()).andReturn();
	...
	public void testgetACFById() throws Exception {
		...
		TestUsers.setCurrentUser("ROLE_ORG_ADMIN", 9999L, 99999L); // the info doesn't matter for role_admin
		...
		// scenario2: org_admin gets back 0, since he belongs to a different org
		TestUsers.setCurrentUser("ROLE_ORG_ADMIN", 10L, 100L); // the info doesn't matter for role_admin
	...
	public void testCreate() throws Exception {
		...
		TestUsers.setCurrentUser("ROLE_PROVIDER", 999999L, 99999L); // the info doesn't matter for role_admin
		mockMvc.perform(post("/acfs/create").contentType(contentType).content(requestJsonAcf))
				.andExpect(status().isUnauthorized());
		...
		assertEquals(acfDb.getLiferayAcfId(), TestUsers.getCurrentUser().getLiferayAcfId());
		assertEquals(acfDb.getLiferayStateId(), TestUsers.getCurrentUser().getLiferayStateId());

		/* scenario 3: role_org_admin */
		acfCreate.setName("Test Create ACF2");
		TestUsers.setCurrentUser("ROLE_ORG_ADMIN", 999999L, 99999L); // the info doesn't matter for role_admin
	...
	public void testAcfEdit() throws Exception {
		...
		TestUsers.setCurrentUser("ROLE_ADMIN", liferayStateIdUsedForTest + 1, liferayAcfIdUsedForTest + 1); // the info
																											// doesn't
																											// matter
																											// for
																											// role_admin
		...
		TestUsers.setCurrentUser("ROLE_ORG_ADMIN", liferayStateIdUsedForTest, liferayAcfIdUsedForTest + 1);
		mockMvc.perform(post(testAcfEditUrl).contentType(contentType).content(requestJsonAcfTest))
				.andExpect(status().isOk());

		// scenario 4: provider for the same acf allowed
		TestUsers.setCurrentUser("ROLE_PROVIDER", liferayStateIdUsedForTest, liferayAcfIdUsedForTest);
		mockMvc.perform(post(testAcfEditUrl).contentType(contentType).content(requestJsonAcfTest))
				.andExpect(status().isOk());

		// scenario 5: provider for differnt acf not allowed
		TestUsers.setCurrentUser("ROLE_PROVIDER", liferayStateIdUsedForTest, liferayAcfIdUsedForTest + 1);
		mockMvc.perform(post(testAcfEditUrl).contentType(contentType).content(requestJsonAcfTest))
				.andExpect(status().isUnauthorized());
	}
